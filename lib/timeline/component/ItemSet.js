...
ItemSet.prototype._onMouseOver = function (event) {
  ...
  if (this.options.showTooltips && title) {
    ...
    fastdom.measure(() => {
      positionX = event.clientX - util.getAbsoluteLeft(container) + container.offsetLeft;
      positionY = event.clientY - util.getAbsoluteTop(container) + container.offsetTop;
    });
    this.popup.setPosition(positionX, positionY);
    this.popup.show();
  } else {
